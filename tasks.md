...
| 6.4 | GitHub Actions build + test | 🤖 | ✅ | `.github/workflows/ci.yml` — build+test and SwiftLint, both green. Simulator UDID discovered at runtime. Needed a **shared scheme** (the autogenerated one lives in gitignored `xcuserdata`). ⚠️ Private repo: macOS minutes bill at **10×**, ~200 usable/month on the free tier. |
