...
| 1.4 | Create private GitHub repo + push | 🤖 | ✅ | **https://github.com/mltechexperts/PhotoStyler** — now **public** (was private; changed 2026-09-06 so Actions is free). History scanned for secrets first: none found. |
...
| 6.4 | GitHub Actions build + test | 🤖 | ✅ | `.github/workflows/ci.yml` — build+test and SwiftLint, both green. Simulator UDID discovered at runtime. Needed a **shared scheme** (the autogenerated one lives in gitignored `xcuserdata`). Repo is public, so Actions minutes are free and unlimited. |
